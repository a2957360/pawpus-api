...
if ($_SERVER["REQUEST_METHOD"] == "GET") {
  ...
  $list = array();
  $stmt = $pdo->prepare("SELECT `momentId`,`momentImage`,`momentTitle`,`momentLike`,`userTable`.`userName`,`userTable`.`userImage` 
                         From `savedTable`
                         LEFT JOIN `momentTable` ON `momentTable`.`momentId` = `savedTable`.`targetId`
                         LEFT JOIN `userTable` ON `momentTable`.`userId` = `userTable`.`userId`
                         WHERE `savedTable`.`userId` = '$loginUserId' AND `targetType` = '2' AND `momentTable`.`momentType` = '0' ".$searchSql." limit $offset, $limit");
  $stmt->execute();
  if($stmt != null){

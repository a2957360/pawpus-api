...
$stmt = $pdo->prepare("SELECT count(*) as `num` From `userTable` WHERE `userPhone` = '$userPhone'");
$stmt->execute();
$order = 0;
if($stmt != null){
    while($row=$stmt->fetch(PDO::FETCH_ASSOC)){
        if($row['num'] > 0){
            echo json_encode(["message"=>"exist phone","errorCode"=>106]);
            exit();
        }
    }
}else{
...
//判断是否短时间发送
$stmt = $pdo->prepare("SELECT `createTime` From `textTable` WHERE `userPhone` = '$userPhone' AND `textType` = '0'");
$stmt->execute();
$order = 0;
if($stmt != null){
...
$stmt = $pdo->prepare("UPDATE `textTable` SET `code` = '$checknumber' WHERE `userPhone` = '$userPhone' AND `textType` = '0'");
$stmt->execute();
if($stmt->rowCount() == 0){
    //textType '0：用户；1：服务者'
    $stmt = $pdo->prepare("INSERT INTO `textTable`(`userPhone`,`code`,`textType`) 
                        VALUES ('$userPhone','$checknumber','0')");
    $stmt->execute();

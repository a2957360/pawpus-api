  ...
  if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    foreach ($cartList as $key => $value) {
      if($value['itemId'] == $itemId && $value['itemOptionId'] == $itemOptionId){
      	$isAdd = true;
        $cartList[$key]['itemQuantity'] = (int)$cartList[$key]['itemQuantity'] + (int)$itemQuantity;
      }
      $itemNumber += (int)$cartList[$key]['itemQuantity'];
    }
    if(!$isAdd){
    	$cartList[] = ["itemId"=>$itemId,"itemOptionId"=>$itemOptionId,"itemQuantity"=>$itemQuantity];
      	$itemNumber += (int)$itemQuantity;
    }
    ...
    $stmt->execute();
    if($stmt->rowCount() == 0){
      $stmt = $pdo->prepare("INSERT INTO `cartTable`(`userId`,`itemList`,`itemNumber`) 
                            VALUES ('$userId','$uploadcartList','$itemNumber')");
      $stmt->execute();
    }
